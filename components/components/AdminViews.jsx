...
{VIEWS_SQL}
  ...
  return (
    <>
      ...
      {data.events > 0 && (
        <p className="mt-4 text-[12.5px] leading-relaxed text-navy-soft">
          Bốn con số trên chỉ đếm <strong className="text-navy">lượt mở trang</strong>. Ngoài ra còn{" "}
          <strong className="text-navy">{nf.format(data.events)}</strong> ghi nhận khác — section
          được đọc tới, hoạt động được mở, link liên hệ được bấm, độ cuộn và thời gian đọc — nằm ở
          các bảng bên dưới.
        </p>
      )}
