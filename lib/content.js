 ...
 *
 *  Nothing here is fixed: every heading, button and small label lives in
 *  `defaultSettings.ui` and can be rewritten in /admin → Text.
 * ============================================================================
 */
...
export const defaultSettings = {
  ...
  /* ABOUT ME — a timeline. Each milestone is one year, a few lines, and the
     photographs that go with it. Edited in /admin → About.
       { id, year, title, text, images: [url] }                              */
  about: {
    eyebrow: "Ngoài hồ sơ",
    title: "Những thứ khiến tôi trở thành tôi",
    lead:
      "Những chuyện không nằm gọn trong một dòng CV — nơi tôi đã đi qua, thứ tôi cứ quay lại mãi, và những người đã ở đó cùng tôi.",
    milestones: [],
  },

  /* BLOG — a personal paper. Posts live in their own database table; this is
     only the framing around them, and it is in Vietnamese because that is the
     language the posts are written in. -------------------------------------- */
  blog: {
    eyebrow: "Ghi chép",
    title: "Blog",
    lead:
      "Có những điều tôi chỉ hiểu rõ sau khi viết chúng ra. Đây là chỗ tôi viết dài hơn một dòng CV — về fintech, về chuyện học, và về những thứ chưa gọi được tên.",
    categories: [
    ...
    ui: {
      allLabel: "Tất cả",
      empty: "Chưa có bài nào trong mục này.",
      readMore: "Đọc tiếp",
  ...
  /* Manual running order, per section: { academic: [entryId, …], … }.
     Set with the ↑ ↓ buttons in /admin → Activities. Activities listed here
     come first, in this order; everything else follows, newest first. Kept in
     the settings rather than in a database column so that ordering needs no
     migration and no SQL. --------------------------------------------------- */
  entryOrder: {},

  /* Which keyword pages each activity appears on, set with the tick boxes in
     /admin → Activities:  { "<activity id>": ["excellence", "leadership"] }
     An activity that is not listed here inherits its section's keyword, the
     way every activity used to. See keywordsForEntry() in lib/settings.js. */
  entryKeywords: {},

  /* The running order of the sections on the portfolio, as a list of section
     ids — set with the ↑ ↓ buttons in /admin → Sections. Empty means "group
     them by keyword", which is how the portfolio started out. */
  categoryOrder: [],

  /* The two motion extras, both switchable from /admin → Text.
     smoothScroll: the page eases instead of jumping with the wheel.
     cursor:       the small dot and trailing ring that replace the pointer.
     Both are ignored on touch screens, on /admin, and whenever the visitor
     has asked their system for reduced motion. ---------------------------- */
  motion: { smoothScroll: true, cursor: true },

  /* Full-width photographs placed between the sections of a page. Add as many
     as you like in /admin → Images → Photo bands. Each one is:
       { id, src, caption, page: "home" | "portfolio",
         size: "wide" | "tall", width: "full" | "inset", x, y, zoom }        */
  bands: [],

  /* Every other label on the site — page headings, button text, the small
     words above a section. All of it is editable in /admin → Text. --------- */
  ui: {
    home: {
      cta: "View the portfolio",
      ctaSecondary: "Get in touch",
      scroll: "Scroll",
      /* The small line above the two cards linking to About me and the blog. */
      moreLabel: "Elsewhere on this site",
      keywordCta: "Open",
    },
    portfolio: {
      eyebrow: "The record",
  ...
  /* True once the CV activities have been copied into the database. The
     portfolio then reads only from the database. ------------------------- */
  seedImported: false,
};

/* The labels above, as a plain object, for anything that needs them before
   the database has been read. Live pages use settings.ui instead. */
export const ui = defaultSettings.ui;
